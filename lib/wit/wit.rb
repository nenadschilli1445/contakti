...
class Wit
  ...
  def delete_utterances(utterance)
    req(logger, @access_token, Net::HTTP::Delete, "/utterances", {}, utterance)
  end
    ...
    Net::HTTP.start(uri.host, uri.port, {:use_ssl => uri.scheme == 'https'}) do |http|
      ...
      if rsp.code.to_i != 200
        if (json.is_a?(Hash) and json.has_key?('error'))
          raise RequestError.new("Wit responded with an error: #{json['error']}", json['code'])
        else
          raise RequestError.new("Wit responded with an error: #{json}")
        end
