$ ->
  $el = $('#task_mark_as_done_modal')
  open_btn = null

  show = (e) ->
    open_btn = $(e.relatedTarget)

  ok = (e) ->
    open_btn.trigger 'contakti:mark-done-ok', result: $el.find('div.select-task-result :checked').val()

  $el
    .on('show.bs.modal', show)
    .on('click', '.mark-done-ok', ok)
...
deleteTemplate = (form_prefix) ->
  ...
  $(document).on 'keyup', '#autoreply_text .sms-message-input', ->
  ...
  $(document).on 'change', '#manager_template_id', ->
    selectedId = $(this).val()
    template = gon.managerTemplates.filter((t) -> t.id.toString() == selectedId)[0]
    if template
      if $(this).parents('#chat_autoreply_text').length > 0 
        ...
        $('#manager_template_title').val template.title
