...
window.Danthes = class Danthes
  ...
  # Methods for setting,removing and clearing any additional properties that should be added to incoming or outgoing messages
  # All added properties will be merged in the incoming/outgoing messages
  @addIncomingProperties: (properties) =>
    if typeof(properties) == "object"
      for key of properties
        @incomingProperties[key] = properties[key]

  @addOutgoingProperties: (properties) =>
    if typeof(properties) == "object"
      for key of properties
        @outgoingProperties[key] = properties[key]

  @removeIncomingProperty: (property) =>
    delete @incomingProperties[property] if typeof(property) == "string"

  @removeOutgoingProperty: (property) =>
    delete @outgoingProperties[property] if typeof(property) == "string"

  @getIncomingPropertues: =>
  ...
  @fayeExtension:
    incoming : (message, callback) =>
      for key of @incomingProperties
        message[key] = @incomingProperties[key]
      @debugMessage "incoming message #{JSON.stringify(message)}"
      callback(message)
    outgoing : (message, callback) =>
      ...
      for key of @outgoingProperties
        message[key] = @outgoingProperties[key]
  ...
  @connectToFaye: ->
    if @server && Faye?
      ...
      @fayeClient.bind 'transport:down', =>
        window.userDashboardApp.wsTransportDown() if window.userDashboardApp
      @fayeClient.bind 'transport:up', =>
        window.userDashboardApp.wsTransportUp() if window.userDashboardApp
        window.userDashboardApp.initialise() if window.userDashoardApp
